...
#Grid creation
up_gr = collect(5:10:300)
down_gr = collect(5:10:300)
up_am = collect(5:10:150)
down_am = collect(5:10:150)
prcnt_increase = collect(120:10:300)
...
iterations = 5000
data = zeros(iterations,8)
for i = 1:iterations
    prcnt_increase_gr = BS.sample(prcnt_increase)
    prcnt_increase_am = BS.sample(prcnt_increase)

    x1_gr_up = BS.sample(up_gr)
    x1_gr_down = BS.sample(down_gr)
    x2_gr_up =  (prcnt_increase_gr/100) * x1_gr_up
    x2_gr_down =  (prcnt_increase_gr/100) * x1_gr_down
    x1_am_up = BS.sample(up_am)
    x1_am_down = BS.sample(down_am)
    x2_am_up =  (prcnt_increase_am/100) * x1_am_up
    x2_am_down =  (prcnt_increase_am/100) * x1_am_down


    range_gr = [min_gr+x1_gr_up max_gr-x1_gr_down;min_gr+x2_gr_up max_gr-x2_gr_down]
    range_am = [min_am+x1_am_up max_am-x1_am_down;min_am+x2_am_up max_am-x2_am_down]
...
data[i,3] = x1_gr_up
data[i,4] = x1_gr_down
data[i,5] = prcnt_increase_gr
data[i,6] = x1_am_up
data[i,7] = x1_am_down
data[i,8] = prcnt_increase_am
end
...
max_points = zeros(length(unique_false_positives),8)
...
max_points_ = zeros(length(collect(0:interval:1)),8)
